...
def split_sequence(
    sequence: address_pb2.ExampleFieldSequence
...
def ignore_specialized_concept(
    concepts: Union[List[address_pb2.Concept],
                    List[address_pb2.CompoundConcept]]
...
for country_data in ontology.country_data:
    if country_data.site_examples_preamble_file:
        country_data.site_examples_preamble_html = (''.join(
            open(country_data.site_examples_preamble_file, "r").readlines()))
    if country_data.site_examples_epilog_file:
        country_data.site_examples_epilog_html = (''.join(
            open(country_data.site_examples_epilog_file, "r").readlines()))
    ...
    for sequence in site_example.sequences:
        for field in sequence.fields:
            if field.section not in (
                    address_pb2.ExampleSequenceSection.NAME,
...
for site_example in ontology.site_examples:
    ...
    for sequence in site_example.sequences:
        for field in sequence.fields:
            ...
            compound_concept_frequencies[concepts_key]["countries"].add(
                country)
